...
const init_03 = () => {
    ...
    $('#timer03').text('00:0' + seconds_03);
    $('#likeCount').text(likeCount + '초');
    $('#basicCount').text(basicCount + '초');
...
const startFaceAnalysis_03 = () => {
    ...
    interval_03 = setInterval(async () => {
        if (seconds_03 > 30) {
            ...
            $.ajax({
                url:BASE_URL+"/posture",
                type: "POST",
                contentType: "application/json; charset=utf-8",
                data: params,
                async: false,
                success: function (result) {
                    if (result.payload.result === '좋음') {
                        $('.popup__quick-foot div').attr('class', 'bg-BU');
                        $('.popup__quick-foot div span').attr('class', 'i-like2');
                        $('.popup__quick-foot div span').text('자세좋음');
                        $('#likeCount').text(likeCount + '초');
                        likeCount++;
                    } else if (result.payload.result === '보통') {
                        $('.popup__quick-foot div').attr('class', 'bg-PU');
                        $('.popup__quick-foot div span').attr('class', 'i-basic');
                        $('.popup__quick-foot div span').text('자세보통');
                        $('#basicCount').text(basicCount + '초');
                        basicCount++;
                    } else if (result.payload.result === '나쁨') {
                    ...
                    if (seconds_03 < 10) $('#timer03').text('00:0' + seconds_03);
                    else $('#timer03').text('00:' + seconds_03);
                    seconds_03++;
                },
                error: function () {
    ...
    // 총 시간 과 분석결과 초의 합이 30이 안되는경우 있음 (delay)
}
...
const displayResults_03 =()=>{
    ...
    $("#resBasic").text(basicCount+'초(' + calculatedInterval_03.basic.length + '회)');
    ...
    resDonut_03 = new Chart(ctx08, {
        ...
        data: {
            datasets: [
                {
                    data: [awayCount, likeCount , basicCount, badCount],
                    backgroundColor: [
                        "#9196a9", //자리비움
                        "#46c7ff", //자세좋음
                        "#ff5959", //자세보통
...
const calculateInterval_03 = (currentPosture) => {
    if (currentPosture === previousPosture) {
        ...
        if (previousPosture === '좋음') {
            calculatedInterval_03.like.push(processingInterval_03);
        } else if (previousPosture === '보통') {
            calculatedInterval_03.basic.push(processingInterval_03);
        } else if (previousPosture === '나쁨') {
            calculatedInterval_03.bad.push(processingInterval_03);
        } else if (previousPosture === '자리비움') {
